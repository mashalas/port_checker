...
usage()
{
  ...
  then
    ...
    echo ' '
    echo 'DESCRIPTION'
    echo '        This script is designed to check the availability of network ports (TCP/UDP) and perform actions depending on whether the port is open or not.'
    echo '        Actions are:'
    ...
    echo 'ИМЯ'
    echo '        port_checker.sh [параметры] хост порт'
    echo ' '
    echo 'ОПИСАНИЕ'
    echo '        Этот скрипт предназначен для проверки достурности сетевых портов (TCP/UDP) и выполнения действий в зависимости от того, открыт порт или нет.'
    echo '        Действия:'
    echo '          * записать информацию в файл;'
    echo '          * выполнить команду операционной системы;'
    echo '          * отправить сообщение по электронной почте.'
    echo ' '
    echo 'ПАРАМЕТРЫ'
    echo '        -h | --help                       напечатать эту справку.'
    echo '        -v | --verbose                    вывод дополнительной информации о ходе работы.'
    echo '        -c | --comment                    текстовый комментарий добавляемый в сообщение для файла или письма электронной почты.'
    echo '        -u | --UDP | --udp                проверять UDP-порт вместо TCP.'
    echo '        -s | --sender <sender>            указать отправителя почтового сообщения.'
    echo ' '
    echo '        --log-on-success <filename>       записать сообщение в файл об успешном соединении.'
    echo '        --exec-on-success <command>       выполнить команду в случае успешного соединения.'
    echo '        --mail-on-success <receiver>      отправить сообщение электронной почты на адрес <receiver> в случае успешного соединения (параметр может быть указан несколько раз для отправки нескольким получателям).'
    echo '        --attach-on-success <filename>    прикрепить файл к письму в случае успешного соединения.'
    echo ' '
    echo '        --log-on-fail <filename>          записать сообщение в файл о неудавшемся соединении.'
    echo '        --exec-on-fail <command>          выполнить команду в случае неудавшегося соединения.'
    echo '        --mail-on-fail <receiver>         отправить сообщение электронной почты на адрес <receiver> в случае неудавшегося соединения (параметр может быть указан несколько раз для отправки нескольким получателям).'
    echo '        --attach-on-fail <filename>       прикрепить файл к письму в случае неудавшегося соединения.'
  fi
  echo LANG: $LANG
}
